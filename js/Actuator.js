...
var Actuator = function(options) {
  ...
  this.amplitude = this.options.amplitude === undefined ? 0 : this.options.amplitude;
  this.phase = 0;
  ...
  this.shape = new CANNON.Box(new CANNON.Vec3(WIDTH, HEIGHT, WIDTH));
  this.body = new CANNON.Body({ mass: 1, position: this.options.position });
  this.body.addShape(this.shape);
  this.body.linearDamping = 0.5;
  this.body.angularDamping = 0.5;

  var geometry = new THREE.BoxBufferGeometry(WIDTH * MESH_SCALE, HEIGHT * MESH_SCALE, WIDTH * MESH_SCALE);
...
Actuator.prototype.stepActuation = function(elapsed) {
  this.scale = 1 + ((Math.sin(elapsed * RATE + this.phase * Math.PI) + 1) / 2) * this.amplitude;
...
Actuator.prototype.joinToLocation = function(actuator, joinedPivotPoints, thisTop, otherTop) {
  ...
  var actuatorConstraint = new CANNON.PointToPointConstraint(this.body, pivotA, actuator.body, pivotB);
  joinedPivotPoints.push(actuatorConstraint.pivotA);
  (otherTop ? actuator.topPivotPoints : actuator.bottomPivotPoints).push(actuatorConstraint.pivotB);
  return actuatorConstraint;
};

var closerToTop = function(quaternion, contactPosition) {
  var distanceToTop = contactPosition.distanceTo(quaternion.vmult(TOP_JOIN_POINT));
  var distanceToBottom = contactPosition.distanceTo(quaternion.vmult(BOTTOM_JOIN_POINT));
...
Actuator.prototype.joinTo = function(actuator, contact) {
  ...
  if (thisTop) {
    return this.joinToLocation(actuator, this.topPivotPoints, thisTop, otherTop);
  } else {
    return this.joinToLocation(actuator, this.bottomPivotPoints, thisTop, otherTop);
  }
};
